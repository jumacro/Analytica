...
require_once SYS_DIR.'/logincheck.php';
...
/* connection settings ------------------------------------------------------ */
error_reporting(0);
// check if video analytics is installed properly
if (!db_check()) {
    $dberror = true;
    ...
    $msg .= 'If server data are correct, go <a href="'.ABS_PATH.'install">install video analytics</a>.';
    $msg .= '</p>';
...
if (is_admin()) {
    ...
    <code><pre><?php
            $code  = '<script type="text/javascript" src="'.ABS_PATH.'api/js/va-api.min.js"></script>' . PHP_EOL;
            $code .= '<script type="text/javascript">' . PHP_EOL;
            $code .= 'try { '             . PHP_EOL;
            $code .= '  var options = {trackingServer: "'.TRACKING_SERVER.'"};' .PHP_EOL;
            $code .= '  va2.record(options);'   . PHP_EOL;
