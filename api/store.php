...
$session = db_select(TBL_PREFIX.TBL_SESSION, "id", "id='".$session_id."'");
if (!$session) {
    $session_id = db_insert(TBL_PREFIX.TBL_SESSION, "created_at", "'".date("Y-m-d H:i:s")."'");
} else {
...
if (!$bname) {
    $browserid = db_insert(TBL_PREFIX.TBL_BROWSERS, "name, created_at", "'".$browser->getBrowser()."', '".date("Y-m-d H:i:s")."'");
} else {
    $browserid = $bname['id'];
}
// save OS id
...
if (!$osname) {
    $osid = db_insert(TBL_PREFIX.TBL_OS, "name, created_at", "'".$browser->getPlatform()."', '".date("Y-m-d H:i:s")."'");
} else {
    $osid = $osname['id'];
}
// save domain id
...
if (!$d) {
    $did = db_insert(TBL_PREFIX.TBL_DOMAINS, "domain, created_at", "'".$domain."', '".date("Y-m-d H:i:s")."'");
} else {
    $did = $d['id'];
}
...
$fields  = "user_id,session_id,client_id,cache_id,domain_id,os_id,browser_id,browser_ver,user_agent,";
$fields .= "ftu,ip,scr_width,scr_height,vp_width,vp_height,";
$fields .= "sess_date,sess_time,fps,coords_x,coords_y,clicks,hovered,clicked, created_at";
...
echo json_encode(array('uid' => $uid, 'session_id' => $session_id));
