...
$sql .= '`user_id`      BIGINT                  NULL, ';                        // user id
$sql .= '`session_id`   BIGINT                  NULL, ';                        // session id
$sql .= '`client_id`    VARCHAR(255)            NOT NULL, ';                    // client id
...
$sql .= '`browser_id`   TINYINT       unsigned  NULL, ';                        // client browser (255 different browsers can be tracked)
$sql .= '`browser_ver`  FLOAT(3,1)    unsigned  NULL, ';                        // client browser version
$sql .= '`user_agent`   VARCHAR(255)            NULL, ';                        // full user agent string
$sql .= '`ftu`          TINYINT(1)              NULL, ';                        // first time visitor
$sql .= '`ip`           VARCHAR(15)             NOT NULL, ';                    // user IP
$sql .= '`scr_width`    SMALLINT      unsigned  NULL, ';                        // client screen resolution
$sql .= '`scr_height`   SMALLINT      unsigned  NULL, ';
$sql .= '`vp_width`     SMALLINT      unsigned  NULL, ';                        // client viewport size
$sql .= '`vp_height`    SMALLINT      unsigned  NULL, ';
...
$sql .= '`fps`          TINYINT       unsigned  NULL, ';                        // registration accuracy
$sql .= '`coords_x`     MEDIUMTEXT              NULL, ';                        // mouse coordinates (max 16777215 chars in one mouse trail)
$sql .= '`coords_y`     MEDIUMTEXT              NULL, ';
$sql .= '`clicks`       MEDIUMTEXT              NULL, ';                        // mouse clicks
$sql .= '`hovered`      LONGTEXT                NULL, ';                        // most hovered widgets (max 4294967298 chars)
$sql .= '`clicked`      LONGTEXT                NULL, ';                        // ...and clicked
$sql .= '`created_at`   DATETIME                NULL, ';                        // created at time
$sql .= '`updated_at`   DATETIME                NULL, ';                        // updated time
$sql .= 'PRIMARY KEY (`id`) ';
...
$sql .= '`url`          TEXT                    NOT NULL, ';                    // tracked page url
...
$sql .= '`name`         VARCHAR(100)             NOT NULL, ';                    // OS name
$sql .= '`created_at`   DATETIME                 NULL, ';                        // created at time
$sql .= '`updated_at`   DATETIME                 NULL, ';                        // updated time
$sql .= 'PRIMARY KEY (`id`) ';
$sql .= ') DEFAULT CHARSET utf8';
...
$sql .= '`name`         VARCHAR(255)             NOT NULL, ';                    // browser name
$sql .= '`created_at`   DATETIME                 NULL, ';                        // created at time
$sql .= '`updated_at`   DATETIME                 NULL, ';                        // updated time
$sql .= 'PRIMARY KEY (`id`) ';
$sql .= ') DEFAULT CHARSET utf8';
...
$sql .= '`login`        VARCHAR(255)            NOT NULL, ';                    // user login
$sql .= '`pass`         VARCHAR(60)             NOT NULL, ';                    // user pass
$sql .= '`name`         VARCHAR(255)            NOT NULL, ';                    // user name
$sql .= '`email`        VARCHAR(255)            NOT NULL, ';                    // email
$sql .= '`website`      VARCHAR(255)            NULL,     ';                    // url
...
$ADMINPASS = "admin123";//generate_password();
// insert admin user
if (!db_select(TBL_PREFIX.TBL_USERS, "id", "login = 'admin'")) {
    $sql  = "INSERT INTO ".TBL_PREFIX.TBL_USERS." (role_id, login, pass, name, email, registered)";
    $sql .= " VALUES (1, 'admin', MD5('".$ADMINPASS."'), 'System Administrator', '".$_POST['email']."', NOW())";
    db_query($sql);
}
...
$sql .= '`ext_allowed`  VARCHAR(255)            NULL, ';                        // allowed extensions for this role
$sql .= '`created_at`   DATETIME                NULL, ';                        // created at time
$sql .= '`updated_at`   DATETIME                NULL, ';                        // updated time
$sql .= 'PRIMARY KEY (`id`) ';
$sql .= ') DEFAULT CHARSET utf8';
...
$sql .= '`dir`        VARCHAR(255)             NOT NULL, ';                     // ext dir
...
foreach ($exts as $dir)
{
    ...
    if (!db_select(TBL_PREFIX.TBL_EXTS, "id", "dir = '".$dir."'")) {
        $sql  = "INSERT INTO ".TBL_PREFIX.TBL_EXTS." (dir, priority) VALUES ('".$dir."', 0)";
        db_query($sql);
    }
}
...
$sql .= '`name`         VARCHAR(255)            NOT NULL, ';                    // option name
$sql .= '`value`        VARCHAR(255)            NOT NULL, ';                    // option value
$sql .= '`description`  TEXT                    NULL, ';                        // option description
$sql .= '`created_at`   DATETIME                NULL, ';                        // created at time
$sql .= '`updated_at`   DATETIME                NULL, ';                        // updated time
$sql .= 'PRIMARY KEY (`id`) ';
$sql .= ') DEFAULT CHARSET utf8';
...
echo 'Creating table <em>'.TBL_PREFIX.TBL_SESSION.'</em>: ';

$sql  = 'CREATE TABLE IF NOT EXISTS `'.TBL_PREFIX.TBL_SESSION.'` (';
$sql .= '`id`           BIGINT        unsigned  NOT NULL auto_increment, ';     // session id
$sql .= '`created_at`   DATETIME                NULL, ';                        // created at time
$sql .= '`updated_at`   DATETIME                NULL, ';                        // updated time
$sql .= 'PRIMARY KEY (`id`) ';
$sql .= ') DEFAULT CHARSET utf8';
...
<p>
    Your admin login is <strong>admin</strong>, with password <strong><?=$ADMINPASS?></strong>
    <br />
