...
var va2fn = {
  /**
   * Overrides (va) tracking options object with custom-provided options object
   * @return void
   * @param {object} vaOptionsObj
   * @param {object} customOptionsObj
   * @see <code>vaOpt</code> object either in <code>vaRecord</code> or <code>vaReplay</code> classes
   */
  overrideTrackingOptions: function(vaOptionsObj, customOptionsObj)
  {
    for (var prop in vaOptionsObj)
    {
      if (customOptionsObj.hasOwnProperty(prop) && customOptionsObj[prop] !== null) {
        vaOptionsObj[prop] = customOptionsObj[prop];
      }
    }
  },
   ...
   * @return void
   * @param {object}   vaData    The user's data object
   */
  loadNextMouseTrail: function(vaData)
  {
    if (typeof vaData.api === 'undefined') { vaData.api = "js"; }
    var currTrailPos = this.array.indexOf(vaData.trails, vaData.currtrail);
    // check
    if (currTrailPos < vaData.trails.length - 1) {
      var navigateTo = vaData.trailurl+'?id='+vaData.trails[currTrailPos + 1]+'&api='+vaData.api;
      if (vaData.autoload) {
  ...
  {
    if (!digits) { digits = 2; }
    var exp = 100; // faster, because va2 precision is the same for all computations!
   ...
   * This snippet was developed years ago as 'DOM4CSS', and now lives in harmony with va2.
   */
  widget: {
    /**
     * Concatenation token.
