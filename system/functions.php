...
// database ---
require_once REQUIRED.'/define.db.php';
require_once REQUIRED.'/functions.db.php';

// constants ---
require_once REQUIRED.'/define.php';
require_once REQUIRED.'/messages.php';

//utilities ---
require_once REQUIRED.'/DOMUtil.php';
require_once REQUIRED.'/Browser.php';
require_once REQUIRED.'/KMeans.php';
require_once REQUIRED.'/MouseFeat.php';
require_once REQUIRED.'/Hypernote.php';

//  others ---
require_once REQUIRED.'/functions.array.php';
require_once REQUIRED.'/functions.url.php';

require_once BASE_PATH.'/api/functions.php';
 ...
 * @return  string  SQL query
 */
function get_cache_common_url_by_session($session_id)
{
    $merge = "";
    $cache_ids = [];

    $records = db_select_all(TBL_PREFIX.TBL_RECORDS, "*", "session_id = '".$session_id."'");

    if (!empty($records))
    {
        foreach ($records as $i => $r)
        {
            $cache_ids[] = $r['cache_id'];
            $common = db_select(TBL_PREFIX.TBL_CACHE, "url", "id = '".$r['cache_id']."'");
            $moreId = db_select_all(TBL_PREFIX.TBL_CACHE, "id", "url = '".$common['url']."'");
            // merge values
            $moreId = array_flatten($moreId);
            // set query
